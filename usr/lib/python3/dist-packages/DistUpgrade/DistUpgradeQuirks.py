...
class DistUpgradeQuirks(object):
    ...
    def noblePostUpgrade(self):
        logging.debug("running Quirks.noblePostUpgrade")
        cache = self.controller.cache
        if 'snapd' not in cache:
            logging.debug("package required for Quirk not in cache")
            return
    ...
    def _test_and_fail_on_power8(self):
        ...
        with open("/proc/cpuinfo") as f:
            ...
            if re.search(r"^cpu\s*:\s*POWER8", cpuinfo, re.MULTILINE):
    ...
    def _checkArmCPU(self):
        ...
        with open("/proc/cpuinfo") as f:
            cpuinfo = f.read()
        if re.search(r"^Processor\s*:\s*ARMv[45]", cpuinfo,
                     re.MULTILINE):
            return False
        return True
    ...
    def _checkPae(self):
        ...
        with open('/proc/cpuinfo') as f:
            cpuinfo = f.read()
        if re.search(r"^flags\s+:.* pae ", cpuinfo, re.MULTILINE):
            pae = 1
        if not pae:
    ...
    def _test_and_fail_on_bios_with_xfs_boot(self):
        """
        If this system is BIOS and has XFS on /boot, do not allow the upgrade
        to happen (LP: #2039172).
        """
        if os.path.isdir('/sys/firmware/efi'):
            # UEFI boot, nothing to do.
            return
        ...
        if boot_fs == 'xfs' or (boot_fs is None and root_fs == 'xfs'):
            # If we got to this point, then this is a BIOS system, and /boot is
            # XFS or / is XFS and there is no separate /boot partition. Do not
            # allow the upgrade to happen.
    ...
    def _test_and_fail_on_armhf_raspi(self):
        """
        ...
        if (
            'linux-image-raspi' in self.controller.cache and
            self.controller.cache['linux-image-raspi'].is_installed
        ):
            logging.error('Detected Raspberry Pi kernel on armhf')

            di = distro_info.UbuntuDistroInfo()
            releases = {
                r.series: r
                for r in di.get_all('object')
            }
            from_release = releases.get(self.controller.fromDist)

            if from_release is not None:
                updates_end = from_release.eol.strftime('%B %Y')
                updates_end_msg = _(
                    f'Updates for Ubuntu {from_release.version} will '
                    f'continue until {updates_end}.'
                )
            else:
                # This shouldn't happen. But rather than crash the upgrader,
                # just don't show the second part of the message.
                updates_end_msg = ''
            ...
            self._view.error(
                ...
                _(
                    'The Raspberry Pi kernel for Ubuntu 24.04 LTS '
                    'does not support the armhf architecture.\n\n'
    ...
    def _install_t64_replacement_packages(self):
        ...
        for package in self.controller.cache:
            ...
            reverse_depends_list = [
                (dep.dep_type_untranslated, dep.parent_ver)
                for dep in package._pkg.rev_depends_list
            ]

            provides_list = [ver for _, _, ver in package._pkg.provides_list]

            for ver in provides_list:
                if ('Replaces', ver) not in reverse_depends_list:
                    continue

                if (
                    ('Breaks', ver) not in reverse_depends_list and
                    ('Conflicts', ver) not in reverse_depends_list
                ):
                    continue

                replacement = apt.Package(
                ...
                logging.debug(
                    f'Found replacement {replacement.name} '
                    f'for {package.name}'
                )
                ...
                # We found a replacement. Exit the loop.
                break

            else:
                # We did not find a replacement in the provides list.
                logging.debug(
